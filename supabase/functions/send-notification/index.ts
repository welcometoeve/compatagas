...
const supabaseUrl = "https://rzfprffcjagjztalfjhr.supabase.co";
const supabaseKey =
...
async function sendPushNotification(expoPushToken, message, title) {
...
Deno.serve(async (req) => {
  ...
  if (req.method === "POST") {
    try {
      const { recipientId, message, title } = await req.json();
      console.log(
        `Received notification request for recipient: ${recipientId}`,
      );

      console.log("Fetching recipient's notification token from Supabase");
      ...
      if (error || !data?.notificationToken) {
        console.error("Failed to get recipient token:", error);
        return new Response(
      ...
      console.log(`Retrieved notification token: ${data.notificationToken}`);
